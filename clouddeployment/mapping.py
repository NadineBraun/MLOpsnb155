...
def detect_columns_with_prompt(columns, file, df_sample=None):
    ...
    if col_hash in column_cache:
        return column_cache[col_hash].copy()

    mapping_override_path = os.path.join(os.getcwd(), "mapping_overrides.json")
    if os.path.exists(mapping_override_path):
        with open(mapping_override_path) as f:
            all_mappings = json.load(f)
        if col_hash in all_mappings:
            override = all_mappings[col_hash]
            col_map = {
                "pickup_col": override.get("pickup_col"),
                "dropoff_col": override.get("dropoff_col"),
                "distance_col": override.get("distance_col"),
                "fare_col": override.get("fare_col"),
                "total_col": override.get("total_col"),
                "passenger_col": override.get("passenger_col"),
                "payment_type_col": override.get("payment_type_col"),
                "pickup_long_col": override.get("pickup_long_col"),
                "pickup_lat_col": override.get("pickup_lat_col"),
                "dropoff_long_col": override.get("dropoff_long_col"),
                "dropoff_lat_col": override.get("dropoff_lat_col")
            }
            column_cache[col_hash] = col_map.copy()
            print(f"[INFO] Mapping für Hash {col_hash} aus overrides.json übernommen.")
            return col_map
        else:
            print(f"[INFO] Kein Mapping für Hash {col_hash} vorhanden – interaktive Zuordnung wird verwendet.")

    # Fallback – Interaktive Eingabe
    col_map = {
        "pickup_col": None,
        "dropoff_col": None,
        "distance_col": None,
...
def run_mapping_for_years(years, folder="data"):
    results = []
